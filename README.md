...
- [⚙️ 兼容性](#️-兼容性)
- [🔧 技术栈](#-技术栈)
- [⬆️ 升级说明（v0.11.x → v0.13.0）](#️-升级说明v011x--v0130)
- [📄 许可证](#-许可证)
...
| 便笺       | 按**工作区共享**的便笺（同一工作区目录下所有会话共用一份内容，本地持久化）；输入框底部拖拽手柄可调整高度（实时持久化），5000 字上限与字符计数，右侧圆点指示内容存在；可一键添加便笺全文至对话输入框；折叠/展开状态持久化                                                                                                                                                                                                          |
...
- **便笺模块**：按**工作区共享**的便笺——以会话所在工作区目录为键保存（同一工作区目录下的所有会话共用一份内容，含输入框高度），本地持久化；删除或归档其中任一/全部会话都不影响该工作区其它会话的内容，下次在同一工作区新建会话时内容依旧存在；输入框底部拖拽手柄可拖拽调整高度（实时持久化），5000 字上限与字符计数显示；右侧圆点指示便笺内容存在；可一键添加便笺全文至对话输入框（新建会话页会发送到该工作区最近的会话）；模块可折叠/展开（状态持久化）。未归属工作区（无 cwd）的会话降级为会话级隔离，不与其它会话共享。
