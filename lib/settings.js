...
export const MODULE_KEYS = Object.freeze(["context", "git", "subagents", "tasks", "mcp"]);
...
export function defaultSettings() {
	return { version: 1, modules: defaultModules(), mcpDisabled: {} };
}
...
export function normalizeSettings(raw) {
	...
	if (isPlainObject(raw.mcpDisabled)) {
		for (const [sessionId, servers] of Object.entries(raw.mcpDisabled)) {
			if (typeof sessionId !== "string" || sessionId === "") continue;
			out.mcpDisabled[sessionId] = Array.isArray(servers)
				? [...new Set(servers.filter((item) => typeof item === "string" && item !== ""))]
				: [];
		}
	}
	return out;
