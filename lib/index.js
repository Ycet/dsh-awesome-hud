...
export function apply(ctx) {
	...
	const handlers = {
		...
		async "git/graph"(payload) {
			...
			if (subprocess === undefined) throw Object.assign(new Error("命令执行服务不可用"), { status: 503 });
			const cwd = resolveSessionCwd(sessions, sessionId);
			const result = await runGit(subprocess, cwd, ["log", "--graph", "--oneline", "--decorate", "--all", "-n", "80", "--color=never"]);
			if (result.exitCode !== 0) return { graph: null, error: result.stderr.trim() || "git log 失败" };
		...
		async "compact"(payload) {
			...
			if (agents === undefined || compaction === undefined) {
				throw Object.assign(new Error("压缩服务不可用"), { status: 503, code: "unavailable" });
			}
			...
			try {
				...
				const code = error?.name === "ManualCompactionError" && typeof error?.code === "string" ? error.code : "internal";
				if (code === "busy") throw Object.assign(new Error("会话正在运行，请稍后再试"), { status: 409, code: "busy" });
				throw Object.assign(new Error(error instanceof Error ? error.message : String(error)), { status: 500, code });
			}
		},
	};
