...
export async function generateCommitMessage(ctx, subprocess, cwd, provider, model, sessionId) {
	...
	// 当前会话最近对话上下文（用户直接消息与助手消息；供理解改动意图、采纳用户指定的备注）
	let sessionContext = "";
	if (typeof sessionId === "string" && sessionId !== "") {
		try {
			const session = ctx.get("sessions")?.get(sessionId);
			if (session !== undefined) sessionContext = extractSessionContext(session.events);
		} catch { /* 上下文不可用时忽略 */ }
	}
	const system = [
...
export function apply(ctx) {
	...
	const handlers = {
		...
		async "git/commit-ai"(payload) {
			...
			if (subprocess === undefined) throw Object.assign(new Error("命令执行服务不可用"), { status: 503 });
			const cwd = resolveSessionCwd(sessions, sessionId);
			const message = await generateCommitMessage(ctx, subprocess, cwd, payload?.provider, payload?.model, sessionId);
			return { message };
		},
