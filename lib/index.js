...
/**
 * 从会话工作区向上逐级收集 AGENTS.md（最近的优先），再补用户级 ~/.dsh/AGENTS.md。
 * 合并内容（总长上限 MAX_AGENTS_BYTES）；找不到返回空字符串。
 */
export async function collectAgentsDocs(cwd) {
	const paths = [];
	let dir = cwd;
	for (;;) {
	...
	const userLevel = join(homedir(), ".dsh", "AGENTS.md");
	if (!paths.includes(userLevel)) {
		try {
			await access(userLevel, fsConstants.R_OK);
			paths.push(userLevel);
		} catch { /* 用户级不存在 */ }
	}
	let total = 0;
	const parts = [];
	for (const path of paths) {
		try {
			...
			const rest = MAX_AGENTS_BYTES - total;
			if (rest <= 0) break;
			parts.push(text.length > rest ? text.slice(0, rest) : text);
...
/** 用当前会话模型生成提交信息（llm.stream 拼接 text-delta）。失败抛错。 */
export async function generateCommitMessage(ctx, subprocess, cwd, provider, model) {
	...
	const agents = await collectAgentsDocs(cwd);
	const system = [
		"You are a git commit message generator embedded in DeepSeek Harness.",
		"The repository or user may document commit conventions in AGENTS.md files; follow them exactly when present:",
		agents.trim() === "" ? "(no AGENTS.md content found in this workspace; use a concise conventional one-line message)." : agents,
		"Reply with ONLY the commit message itself: one line, no markdown fences, no prefixes, no commentary.",
	].join("\n\n");
	const prompt = [
		"Generate one concise git commit message (one line) for the following staged changes (git diff --staged):",
		"",
		body,
	].join("\n");
	const controller = new AbortController();
	...
	try {
		for await (const chunk of llm.stream({
			...
			else if (chunk.type === "finish" && chunk.reason.kind !== "success") {
	...
	if (message.startsWith('"') && message.endsWith('"') && message.length >= 2) message = message.slice(1, -1).trim();
	if (message === "") throw Object.assign(new Error("模型返回了空提交信息"), { status: 502 });
	return message;
}
...
export function apply(ctx) {
	...
	const handlers = {
		...
		async "git/revert-all"(payload) {
			return withGitWrite(ctx, payload, async (subprocess, cwd, before) => {
				const targets = before.files.filter((file) => file.unstaged === true && file.status !== "?").map((file) => file.path);
				if (targets.length === 0) throw Object.assign(new Error("当前没有可撤销的未暂存变更"), { status: 400 });
				// 分批执行（每批 500 路径），避免单命令参数超长
		...
		async "git/commit-ai"(payload) {
			...
			if (subprocess === undefined) throw Object.assign(new Error("命令执行服务不可用"), { status: 503 });
			const cwd = resolveSessionCwd(sessions, sessionId);
			const message = await generateCommitMessage(ctx, subprocess, cwd, payload?.provider, payload?.model);
			return { message };
		},
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				} else if (method === "git" || method === "git/graph" || method === "git/stage" || method === "git/unstage" || method === "git/revert-file" || method === "git/revert-all" || method === "git/commit" || method === "git/commit-ai" || method === "subagents" || method === "mcp" || method === "mcp/toggle" || method === "plans" || method === "compact") {
