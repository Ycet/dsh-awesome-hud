...
import { parsePorcelainZ, parseNumstat, mergeNumstat, normalizeBranch, parseBranchList, sanitizeGitBranch, parseCheckoutConflict, sanitizeGitCommit, parseMergeLocalChanges, parseMergeConflictFiles } from "./git.js";
...
export function apply(ctx) {
	...
	const handlers = {
		...
		async "git/graph"(payload) {
			...
			return { ...slicePage(parseGitLogOutput(result.stdout), GRAPH_PAGE_SIZE) };
		},
		...
		async "git/create-branch"(payload) {
			// 从当前 HEAD 创建新分支并切换；分支名经严格校验 + 重名预检
			const branch = sanitizeGitBranch(payload?.branch);
			if (branch === null) throw new Error("缺少合法的分支名");
			return withGitWrite(ctx, payload, async (subprocess, cwd, before) => {
				const raw = await runGit(subprocess, cwd, ["for-each-ref", "--format=%(refname:short)", "refs/heads"]);
				...
				const result = await runGit(subprocess, cwd, ["checkout", "-b", branch]);
				if (result.exitCode !== 0) throw Object.assign(new Error(result.stderr.trim() || "创建分支失败"), { status: 400 });
			});
		},
