...
/**
 * 把解析后的提交数组转为「最近在前」的历史提交标题列表文本
 * （供 AI 生成提交信息作格式/风格参考）。每条一行；空输入或无有效标题返回 ""。
 * 总长超过 maxBytes 时截断：保留靠前的完整条目，最后一条装不下则截短该条。
 */
export function historySubjectsText(commits, maxBytes = 4096) {
	if (!Array.isArray(commits)) return "";
	if (typeof maxBytes !== "number" || !Number.isFinite(maxBytes) || maxBytes <= 0) maxBytes = 4096;
	const lines = [];
	for (const commit of commits) {
		if (commit === null || typeof commit !== "object") continue;
		if (typeof commit.subject !== "string") continue;
 ...
 * 返回 {message, warn}：warn 为 "version-not-incremented" 表示重试后版本校验仍不达标（已原样返回并交由前端提示）；其余情况 warn 为 null。失败抛错。 */
export async function generateCommitMessage(ctx, subprocess, cwd, provider, model, sessionId) {
	...
	// 仓库历史提交（当前分支 HEAD 最近 10 条单行标题）作格式参考；
	// 与 graph 同一 git log 格式与解析器；任一步失败（非仓库/权限/无历史）静默跳过
	let historyText = "";
	try {
		const historyRaw = await runGit(subprocess, cwd, ["-c", "core.quotepath=false", "log", "--topo-order", "-n", "10", "--pretty=format:%H%x1f%P%x1f%D%x1f%s%x1f%ar"]);
		if (historyRaw.exitCode === 0) historyText = historySubjectsText(parseGitLogOutput(historyRaw.stdout), HISTORY_MAX_BYTES);
	} catch { /* 历史不可用时忽略 */ }
	const system = [
		...
		historyText === ""
			? ""
			: `Recent commit message history of this repository (most recent first) — use it as a STYLE reference only; the local convention above always takes precedence:\n${historyText}`,
		sessionContext === ""
			? ""
	...
	const attempt = async (feedback) => {
		...
		if (feedback !== null) {
			messages.push(
				{ role: "assistant", content: [{ type: "text", text: feedback.output }] },
	...
	if (!exempt(message)) {
		...
		if (!validation.ok) {
			...
			if (!exempt(message)) {
				...
				if (!validation.ok) return { message, warn: "version-not-incremented" };
			}
		}
	}
