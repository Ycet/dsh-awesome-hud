...
/** 用当前会话模型生成提交信息（按内置本地 git 提交规范，llm.stream 拼接 text-delta）。失败抛错。 */
export async function generateCommitMessage(ctx, subprocess, cwd, provider, model) {
	...
	const system = [
		...
		pkg === null
			...
			: `The project name is ${pkg.name} and its CURRENT version is ${pkg.version}. This is a development project, so the message MUST include the project name and a version: it must match exactly [YYMMDD] ${pkg.name} v<new-version>：修改内容 (e.g. [${today}] ${pkg.name} v${pkg.version}：修复了登录页面的样式问题). Determine <new-version> strictly per the versioning rules above: breaking change(s) bump the MAJOR, backward-compatible new feature(s) bump the MINOR (patch resets to 0), otherwise bump the PATCH.`,
