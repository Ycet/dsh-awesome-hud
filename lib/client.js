...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 菜单可隐藏的模块（会话模块恒展示）
		const MODULE_KEYS = ["context", "git", "subagents", "tasks", "mcp", "balance"];
		...
		function defaultModules() {
			return { context: true, git: true, subagents: true, tasks: true, mcp: true, balance: true };
		}
		...
		function apply(ctx) {
			...
			function SettingsMenu(props) {
				...
				return react.createElement("div", { className: "hud-settings-pop", style: props.style },
					...
					react.createElement("div", { className: "hud-settings-actions" },
						...
						react.createElement("button", { type: "button", className: "hud-settings-btn", "data-kind": "confirm", onClick: () => props.onConfirm(draft) }, t("settingsConfirm"))));
			}
			...
			function SessionModule(props) {
				...
				if (menuOpen) {
					...
					menu = react_dom.createPortal(
						react.createElement(SettingsMenu, { modules: props.modules, balanceAvailable: props.balanceAvailable, onConfirm: (draft) => { props.onModulesChange(draft); setMenuOpen(false); }, onCancel: () => setMenuOpen(false), style }),
						document.body
					);
				}
			...
			function GitGraphView(props) {
				...
				const renderLane = (row, rowH, open) => {
					...
					shapes.push(react.createElement("circle", {
						...
						fill: laneOf(row.lane),
						stroke: open ? "var(--hud-surface-bg)" : "none",
									...
									react.createElement("div", { className: "hud-git-content" },
										react.createElement("button", {
											...
											commit.refs.length === 0 ? null : react.createElement("span", { className: "hud-git-refs" },
												commit.refs.map((ref) => react.createElement("span", { className: "hud-git-ref", "data-kind": ref.kind, key: `${ref.kind}:${ref.text}`, title: ref.text },
					...
					props.fold ? null : react.createElement("div", { className: "hud-module-body" },
						props.todos.map((item, index) => {
							...
							return react.createElement("div", { className: "hud-task-row", "data-done": isDone, key: `${item.content}-${index}` },
								react.createElement("span", { className: "hud-task-icon" }, react.createElement(Icon, { name: isDone ? "tasks-done" : "tasks-todo", keepColors: isDone })),
			...
			function BalanceModule(props) {
				...
				const jump = () => { window.open(DEEPSEEK_PLATFORM_URL, "_blank", "noopener"); };
				// 仅展示单个数值：DeepSeek 余额 = 充值余额 + 赠送余额
				const total = (balance.data?.toppedUp ?? 0) + (balance.data?.granted ?? 0);
				return react.createElement("div", { className: "hud-section" },
					react.createElement("div", { className: "hud-module-head" },
						react.createElement(Icon, { name: "balance" }),
						react.createElement("span", { className: "hud-module-title" }, t("balanceModule")),
						react.createElement("button", { type: "button", className: "hud-btn-small hud-git-btn", onClick: jump }, t("balanceJump"))),
					react.createElement("div", { className: "hud-module-body" },
						balance.error === null
							? react.createElement("div", { className: "hud-row" },
								react.createElement("span", { className: "hud-row-label" }, t("balanceTotal")),
								react.createElement("span", { className: "hud-balance-value" }, fmtBalanceMoney(total, balance.data?.currency)))
			...
			function HudPanel() {
				...
				// available=false（未安装插件/未配置令牌）→ 模块与设置行整体隐藏；
				...
				react.useEffect(() => {
					...
					refreshBalance();
					const timer = window.setInterval(refreshBalance, 60000);
				...
				// 滚动条只在面板上下滑动时展示：滚动中标记 data-scrolling，停止 600ms 后移除
				const onPanelScroll = () => {
					const el = bodyRef.current;
					if (el === null) return;
					el.dataset.scrolling = "1";
					if (scrollTimerRef.current !== null) window.clearTimeout(scrollTimerRef.current);
					scrollTimerRef.current = window.setTimeout(() => { delete el.dataset.scrolling; }, 600);
				};
				react.useEffect(() => () => {
					if (scrollTimerRef.current !== null) window.clearTimeout(scrollTimerRef.current);
				}, []);
				...
				return react.createElement("section", {
					...
					react.createElement("div", { className: "hud-panel-body", ref: bodyRef, onScroll: onPanelScroll },
						react.createElement(SessionModule, {
							...
							balanceAvailable: balance !== null && balance.available === true,
						...
						activeModules.balance === false || balance === null || balance.available !== true ? null : react.createElement(BalanceModule, { balance }),
