	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				tag.textContent += ".hud-btn-ai{background:var(--hud-surface-bg)}.hud-btn-ai:hover:not(:disabled){background:color-mix(in srgb,var(--dsw-alias-label-primary) 8%,transparent)}.hud-btn-danger,.hud-git-menu-confirm{border:1px solid var(--dsw-alias-state-error-primary);background:var(--hud-surface-bg);color:var(--dsw-alias-state-error-primary)}.hud-btn-danger:hover:not(:disabled),.hud-git-menu-confirm:hover:not(:disabled){background:color-mix(in srgb,var(--dsw-alias-state-error-primary) 10%,var(--hud-surface-bg));filter:none}";
				tag.textContent += ".hud-git-menu-cancel{border:1px solid var(--dsw-alias-label-primary);background:var(--hud-surface-bg);color:var(--dsw-alias-label-primary)}.hud-git-menu-cancel:hover:not(:disabled){background:color-mix(in srgb,var(--dsw-alias-label-primary) 8%,var(--hud-surface-bg))}";
			...
			function GitGraphView(props) {
				...
				const openMenu = (event, commit) => {
					...
					const margin = 8;
					const menuW = 238;
					const menuH = 8 * 31 + 48; // 3 项复制 + 合并 + 3 项 reset + 2 条分隔线
				...
				const renderMergeItem = () => {
					...
					const disabled = mergeBusy || resetBusy || menu.confirmMode !== null || onMergeCommit === null;
					return react.createElement("button", {
				...
				const renderResetItem = (mode, labelKey) => {
					...
					const disabled = resetBusy || mergeBusy || menu.reset?.allowed !== true || menu.confirmMode !== null || onResetCommit === null;
			...
			function HudPanel({ useSessionPendingInteraction = useNoPendingInteraction }) {
				...
				react.useEffect(() => {
					...
					const conversationRoot = sessionHost?.closest("[data-phase]") ?? null;
					const conversationBody = conversationRoot?.querySelector("[data-conversation-scroll]")?.parentElement ?? null;
			...
			ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				...
				return react.createElement(react.Fragment, null,
					open
						? react.createElement(HudErrorBoundary, null, react.createElement(HudPanel, { useSessionPendingInteraction }))
						: null,
