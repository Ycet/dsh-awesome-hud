	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function GitGraphView(props) {
				...
				const [expanded, setExpanded] = react.useState(commits.length > 0 ? commits[0].hash : null);
				...
				return react.createElement(react.Fragment, null,
					react.createElement("div", { className: "hud-git-graph", onScroll: () => closeMenu(false) },
						react.createElement("div", { className: "hud-git-rows" },
							layout.rows.map((row) => {
								...
								const tip = `${commit.subject}（${commit.hash.slice(0, 7)} · ${commit.date}）`;
