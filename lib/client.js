...
//   2. 悬浮 HUD 面板（shell.overlay）：六个模块 —— 会话 / 上下文窗口 / git /
//      子代理 / 任务 / MCP；与 dsh-better-sidebar 右侧栏互斥协作；
//      布局推挤：#root margin/width 取 max(自有 var, sidebar var)，两者天然互斥。
	...
	factory: (require) => {
		...
		function Icon(props) {
			...
			return react.createElement("span", {
				className: "hud-icon",
				"aria-hidden": "true",
				style: { width: 16, height: 16, display: "inline-flex", flex: "none", color: "var(--dsw-alias-label-secondary)" },
		...
		// —— 样式（仅使用 Theme provider 已核实的 --dsw-alias-* token；进度条用 success/warn/error）——
		const css = "#root{margin-right:max(var(--dsh-awesome-hud-width,0px),var(--dsh-sidebar-width,0px));width:calc(100% - max(var(--dsh-awesome-hud-width,0px),var(--dsh-sidebar-width,0px)));transition:margin-right var(--ds-transition-duration-slow,.2s) var(--ds-ease-in-out,ease),width var(--ds-transition-duration-slow,.2s) var(--ds-ease-in-out,ease)}.hud-btn{border:1px solid var(--dsw-alias-border-l2);width:32px;height:32px;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border-radius:18px;justify-content:center;align-items:center;gap:4px;padding:0;font-size:13px;line-height:20px;display:inline-flex}.hud-btn:hover:not(:disabled){background:var(--dsw-alias-bg-layer-1)}.hud-btn[data-open=true]{border-color:color-mix(in srgb,var(--dsw-alias-brand-primary) 55%,var(--dsw-alias-border-l2));color:var(--dsw-alias-brand-primary)}.hud-panel{position:fixed;right:8px;width:320px;max-height:80vh;display:flex;flex-direction:column;box-sizing:border-box;overflow:hidden;background:var(--dsw-alias-bg-overlay);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;color:var(--dsw-alias-label-primary);font-size:13px;line-height:19px;user-select:none}.hud-panel-body{overflow-y:auto;overscroll-behavior:contain;display:flex;flex-direction:column;gap:10px;padding:10px}.hud-module{border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-alias-bg-layer-1);overflow:hidden;flex:none}.hud-module-head{display:flex;align-items:center;gap:8px;padding:8px 10px;min-height:20px;cursor:default;position:relative}.hud-module-head[data-fold=true]{cursor:pointer}.hud-module-title{font-weight:600;font-size:13px;line-height:18px;color:var(--dsw-alias-label-primary);min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1 1 auto}.hud-module-count{flex:none;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}.hud-module-pill{flex:none;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;padding:0 8px;font-size:11px;line-height:18px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-2)}.hud-module-fold{flex:none;width:16px;height:16px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:0;padding:0;display:inline-flex;align-items:center;justify-content:center;transition:transform .15s var(--ds-ease-in-out,ease)}.hud-module-fold[data-fold=true]{transform:rotate(-90deg)}.hud-module-body{padding:4px 10px 10px;display:flex;flex-direction:column;gap:6px}.hud-row{display:flex;align-items:flex-start;gap:8px;padding:4px 0;min-width:0}.hud-row-label{min-width:0;flex:1 1 auto;overflow-wrap:anywhere;color:var(--dsw-alias-label-primary)}.hud-chip{flex:none;border-radius:999px;padding:0 8px;font-size:11px;line-height:18px;color:#fff;border:1px solid transparent;background:var(--dsw-alias-bg-layer-2)}.hud-chip[data-kind=busy]{background:var(--dsw-alias-brand-primary)}.hud-chip[data-kind=done]{background:var(--dsw-alias-state-success-primary)}.hud-chip[data-kind=warn]{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary)}.hud-chip[data-kind=idle]{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary)}.hud-session-meta{display:flex;flex-direction:column;gap:4px;padding:0 10px 10px}.hud-session-name-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.hud-session-name{font-weight:600;line-height:20px;overflow-wrap:anywhere;min-width:0;flex:1 1 auto}.hud-model-row{display:inline-flex;align-items:center;gap:6px;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;min-width:0}.hud-model-chip{border-radius:999px;padding:0 8px;font-size:11px;line-height:18px;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);flex:none}.hud-model-dot{width:8px;height:8px;border-radius:50%;background:var(--dsw-alias-brand-primary);flex:none;display:inline-block}.hud-context-row{display:flex;align-items:center;gap:8px;padding:0 10px 10px}.hud-bar{flex:1 1 auto;height:8px;border-radius:999px;background:var(--dsw-alias-bg-layer-2);overflow:hidden}.hud-bar-fill{height:100%;border-radius:999px;transition:width .3s var(--ds-ease-in-out,ease)}.hud-breakdown{display:flex;align-items:baseline;gap:6px;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;padding:0 10px 10px}.hud-btn-small{flex:none;border:1px solid var(--dsw-alias-border-l2);background:0 0;color:var(--dsw-alias-label-primary);border-radius:999px;padding:2px 10px;font-size:12px;line-height:18px;cursor:pointer;display:inline-flex;align-items:center;gap:4px}.hud-btn-small:hover:not(:disabled){background:var(--dsw-alias-bg-layer-1)}.hud-btn-small:disabled{opacity:.55;cursor:not-allowed}.hud-file-row{display:flex;align-items:baseline;gap:8px;font-size:12px;line-height:18px;min-width:0}.hud-file-status{flex:none;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-weight:700;min-width:16px;text-align:center}.hud-file-status[data-kind=M]{color:var(--dsw-alias-brand-primary)}.hud-file-status[data-kind=A]{color:var(--dsw-alias-state-error-primary)}.hud-file-status[data-kind=D]{color:var(--dsw-alias-state-error-primary)}.hud-file-status[data-kind=?]{color:var(--dsw-alias-state-warn-primary)}.hud-file-status[data-kind=R]{color:var(--dsw-alias-brand-primary)}.hud-file-status[data-kind=C]{color:var(--dsw-alias-brand-primary)}.hud-file-path{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:pre;direction:rtl;text-align:left;color:var(--dsw-alias-label-primary)}.hud-file-diff{flex:none;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:18px;color:var(--dsw-alias-label-secondary)}.hud-diff-add{color:var(--dsw-alias-state-success-primary)}.hud-diff-del{color:var(--dsw-alias-state-error-primary)}.hud-git-btn{flex:none;align-self:flex-start}.hud-git-summary{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;padding-top:2px}.hud-subagent-row{display:flex;align-items:center;gap:8px;padding:4px 6px;min-width:0;cursor:pointer;border:0;background:0 0;color:var(--dsw-alias-label-primary);width:100%;text-align:left;font-size:13px;line-height:19px;border-radius:6px}.hud-subagent-row:hover{background:var(--dsw-alias-bg-layer-2)}.hud-subagent-name{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.hud-subagent-arrow{flex:none;color:var(--dsw-alias-label-secondary)}.hud-task-row{display:flex;align-items:flex-start;gap:8px;padding:4px 0;min-width:0}.hud-task-icon{flex:none;width:16px;height:16px;padding-top:2px}.hud-task-name{flex:1 1 auto;min-width:0;overflow-wrap:anywhere}.hud-task-row[data-done=true] .hud-task-name{color:var(--dsw-alias-label-secondary)}.hud-mcp-row{display:flex;align-items:center;gap:8px;padding:4px 0;min-width:0}.hud-mcp-name{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.hud-switch{flex:none;width:34px;height:20px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);position:relative;cursor:pointer;padding:0;transition:background .2s var(--ds-ease-in-out,ease)}.hud-switch[data-on=true]{background:var(--dsw-alias-state-success-primary);border-color:transparent}.hud-switch::after{content:'';position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:#fff;transition:left .2s var(--ds-ease-in-out,ease)}.hud-switch[data-on=true]::after{left:16px}.hud-settings-btn{flex:none;border:0;background:0 0;color:var(--dsw-alias-label-secondary);cursor:pointer;padding:2px;display:inline-flex;border-radius:6px}.hud-settings-btn:hover{background:var(--dsw-alias-bg-layer-2)}.hud-settings-pop{position:absolute;right:8px;top:34px;z-index:60;width:190px;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;background:var(--dsw-alias-bg-overlay);box-shadow:0 6px 24px rgba(0,0,0,.18);padding:8px;display:flex;flex-direction:column;gap:2px}.hud-settings-title{font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);padding:2px 6px}.hud-settings-item{display:flex;align-items:center;gap:8px;padding:4px 6px;border-radius:6px;cursor:pointer;font-size:13px;line-height:19px}.hud-settings-item:hover{background:var(--dsw-alias-bg-layer-1)}.hud-settings-item input{accent-color:var(--dsw-alias-brand-primary)}.hud-modal{position:fixed;inset:0;z-index:2147483001;background:rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center}.hud-modal-card{width:min(680px,calc(100vw - 40px));max-height:70vh;display:flex;flex-direction:column;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-overlay);color:var(--dsw-alias-label-primary);box-shadow:0 10px 40px rgba(0,0,0,.3);overflow:hidden}.hud-modal-head{display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid var(--dsw-alias-border-l1)}.hud-modal-head span{flex:1 1 auto;font-weight:600}.hud-modal-close{border:1px solid var(--dsw-alias-border-l2);background:0 0;color:var(--dsw-alias-label-primary);border-radius:999px;padding:2px 10px;font-size:12px;line-height:18px;cursor:pointer;flex:none}.hud-graph{overflow:auto;padding:10px 12px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;line-height:18px;white-space:pre;color:var(--dsw-alias-label-primary)}.hud-toasts{position:fixed;bottom:12px;right:12px;z-index:2147483002;display:flex;flex-direction:column;gap:8px;max-width:min(420px,calc(100vw - 32px))}.hud-toast{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-overlay);color:var(--dsw-alias-label-primary);border-radius:9px;padding:8px 12px;font-size:13px;line-height:19px;box-shadow:0 4px 16px rgba(0,0,0,.16)}.hud-toast[data-kind=error]{border-color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 60%,var(--dsw-alias-border-l2));color:var(--dsw-alias-state-error-primary)}.hud-toast[data-kind=success]{border-color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 55%,var(--dsw-alias-border-l2))}";
		...
		function apply(ctx) {
			...
			function SettingsMenu(props) {
				...
				return react.createElement("div", { className: "hud-settings-pop" },
			...
			function SessionModule(props) {
				...
				return react.createElement("div", { className: "hud-module", style: { position: "relative" } },
					react.createElement("div", { className: "hud-module-head" },
						...
						menuOpen ? react.createElement(SettingsMenu, { modules: props.modules, onChange: props.onModulesChange }) : null),
			...
			function ContextModule(props) {
				...
				const barColor = ratio === null ? "var(--dsw-alias-border-l2)"
				...
				return react.createElement("div", { className: "hud-module" },
					react.createElement("div", { className: "hud-module-head" },
					...
					react.createElement("div", { className: "hud-breakdown" },
						pressure !== null && pressure.projectedTokens !== undefined
							? react.createElement("span", null, interpolate(t("contextUsed"), { tokens: formatTokens(pressure.projectedTokens) }))
							: null,
			...
			function GitModule(props) {
				...
				const [graphState, setGraphState] = react.useState({ text: "", error: "" });
				const openGraph = async () => {
					setGraphOpen(true);
					setGraphState({ text: "", error: "" });
					try {
						const body = await call("git/graph", { sessionId: props.sessionId });
						if (typeof body.graph !== "string" || body.graph === "") throw new Error(body.error ?? t("gitGraphFailed"));
						setGraphState({ text: body.graph, error: "" });
					} catch (error) {
						setGraphState({ text: "", error: error?.message ?? String(error) });
					}
				};
				let modal = null;
				if (graphOpen) {
					modal = react.createElement("div", { className: "hud-modal", onClick: () => setGraphOpen(false) },
						react.createElement("div", { className: "hud-modal-card", onClick: (event) => event.stopPropagation() },
							react.createElement("div", { className: "hud-modal-head" },
								react.createElement("span", null, t("gitGraph")),
								react.createElement("button", { type: "button", className: "hud-modal-close", onClick: () => setGraphOpen(false) }, t("closeModal"))),
							react.createElement("div", { className: "hud-graph" }, graphState.error !== "" ? graphState.error : graphState.text)));
				}
				...
				return react.createElement(react.Fragment, null,
					react.createElement("div", { className: "hud-module" },
					...
					props.fold ? null : react.createElement("div", { className: "hud-module-body" },
						props.entries.map((entry) => {
							...
							return react.createElement("button", {
								...
								key: entry.id,
								style: { paddingLeft: 6 + (entry.depth - 1) * 14 },
					...
					props.fold ? null : react.createElement("div", { className: "hud-module-body" },
						props.todos.map((item, index) => {
							...
							return react.createElement("div", { className: "hud-task-row", "data-done": isDone, key: `${item.content}-${index}` },
								...
								react.createElement("span", { className: "hud-chip", "data-kind": isDone ? "done" : "warn" }, isDone ? t("taskDone") : t("taskTodo")));
						})));
			}
				...
				return react.createElement("div", { className: "hud-module" },
					...
					props.fold ? null : react.createElement("div", { className: "hud-module-body" },
						props.servers.length === 0
							...
							: props.servers.map((server) => react.createElement("div", { className: "hud-mcp-row", key: server.name },
			...
			function HudPanel() {
				...
				// 面板顶部跟随会话头部
				react.useEffect(() => {
					const el = document.querySelector('[data-slot="conversation.session.header"]');
					if (el === null) return undefined;
					const update = () => {
						const rect = el.getBoundingClientRect();
						setTop(Math.max(8, Math.round(rect.bottom + 8)));
					};
					update();
				...
				const statusMap = {
					...
					approval: ["busy", t("statusApproval")],
					answer: ["busy", t("statusAnswer")],
					subagents: ["busy", t("statusSubagents")],
				};
