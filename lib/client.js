...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 打开 HUD，但尊重 better-sidebar 互斥：侧边栏开着时不立即打开，
		 *  记 pendingOpen 延迟到其关闭（由 watcher 恢复），避免两面板共存。
		 *  用于非用户主动的恢复路径（新会话发起后的 blank 恢复等）。 */
		function openHudDeferred(ctx) {
			const sidebar = ctx.get("betterSidebar");
			if (sidebar !== undefined && (sidebar.getSnapshot().state?.panelOpen ?? false)) {
				pendingOpen = true;
				return;
		...
		function apply(ctx) {
			...
			/** 写入按会话分键的对象存储：map[sessionId] = value（写失败仅内存）。 */
			...
			function GitGraphView(props) {
				...
				const openMenu = (event, commit) => {
					...
					const margin = 8;
					const menuW = 208;
					const menuH = 4 * 31 + 36; // 3 项复制 + 分隔线 + 合并
				...
				const menuOverlay = menu === null ? null : react.createElement("div", { className: "hud-git-menu", role: "menu", ref: menuRef, style: { left: menu.x, top: menu.y } },
					...
					react.createElement("button", {
						...
						disabled: mergeBusy,
						onClick: () => {
							if (menu === null || onMergeCommit === null) return;
			...
			function GitModule(props) {
				...
				const [graphState, setGraphState] = react.useState({ commits: null, error: "", hasMore: false, loadingMore: false, loadError: "" });
				...
				const openGraph = async () => {
					setGraphOpen(true);
					setGraphState({ commits: null, error: "", hasMore: false, loadingMore: false, loadError: "" });
					try {
						const body = await fetchGraphPage(0);
						setGraphState({ commits: body.commits, error: "", hasMore: body.hasMore === true, loadingMore: false, loadError: "" });
					} catch (error) {
						setGraphState({ commits: null, error: error?.message ?? String(error), hasMore: false, loadingMore: false, loadError: "" });
					}
				};
				...
				const doMergeCommit = async (commit) => {
					...
					try {
						...
						try {
							...
							setGraphState({ commits: page.commits, error: "", hasMore: page.hasMore === true, loadingMore: false, loadError: "" });
						} catch { /* 列表刷新失败不阻断成功提示 */ }
					} catch (error) {
						if (error?.code === "local-changes-conflict") {
							const files = Array.isArray(error.files) ? error.files.join("、") : "";
			...
			// —— 「便笺」模块：会话隔离 + localStorage 持久化；底部可拖拽调高度的 textarea +
			//     「添加至对话」把内容追加进当前会话输入框（composer draft），不清空便笺 ——
			...
			function NoteModule(props) {
				const [note, setNote] = react.useState(() => {
					const entry = loadJsonStorage(NOTES_STORAGE_KEY)?.[props.sessionId] ?? {};
					return {
						text: typeof entry.text === "string" ? entry.text : "",
						// v2 数据才信任持久化高度；旧记录（无 v 标记，多为 1:1 默认之前的 auto 小高度）作废 → 重新按 1:1 量取默认
						height: entry.v === 2 && typeof entry.height === "number" && entry.height >= 56 ? entry.height : null,
					};
				});
				...
				const patchNote = (patch) => {
					setNote((prev) => {
						...
						persistSessionField(NOTES_STORAGE_KEY, props.sessionId, { ...next, v: 2 });
						return next;
					});
				};
				// 默认高度 = 宽度（1:1）：无 v2 持久化高度时量取输入框宽度；
				// 挂载即折叠或未布局时跳过（尺寸为 0），ResizeObserver 会在可见尺寸变化时再次触发补偿；
				// 高度调整改为拖动底部手柄（hud-note-resize，mousedown 驱动 mousemove），拖拽中实时持久化（按会话）
				react.useEffect(() => {
					const el = areaRef.current;
					if (el === null) return undefined;
					const observer = new ResizeObserver(() => {
						const width = Math.round(el.offsetWidth);
						if (width <= 0) return; // 未可见/未布局，等待下一次尺寸变化
						setNote((prev) => (prev.height === null ? { ...prev, height: width } : prev)); // 1:1 默认（不写存储，保持「默认」语义）
					});
					observer.observe(el);
					return () => observer.disconnect();
				}, [props.sessionId, props.fold]);
				const startResize = (event) => {
					...
					const onMove = (moveEvent) => {
						...
						setNote((prev) => {
							...
							const updated = { ...prev, height: next };
							persistSessionField(NOTES_STORAGE_KEY, props.sessionId, { ...updated, v: 2 });
							return updated;
						});
					};
					const onUp = () => {
				...
				const onSend = () => {
					...
					if (text === "") return;
					if (!appendToComposer(props.sessionId, text)) toast(t("noteSendFailed"));
				};
				return react.createElement("div", { className: "hud-section" },
					react.createElement(ModuleHead, {
						icon: "note",
			...
			function TodoModule(props) {
				...
				const [todos, setTodos] = react.useState(() => {
					const list = loadJsonStorage(TODOS_STORAGE_KEY)?.[sessionId];
					return Array.isArray(list)
						? list.filter((item) => item !== null && typeof item === "object" && typeof item.text === "string")
						: [];
				});
				...
				const persist = (next) => {
					setTodos(next);
					persistSessionField(TODOS_STORAGE_KEY, sessionId, next);
				};
				const addTodo = () => {
					const text = draft.trim();
					if (text === "") return;
				...
				// 删除直接生效（无二次确认弹窗）
				const removeTodo = (id) => persist(todos.filter((value) => value.id !== id));
				// 清空所有已完成待办（弹窗内点击确认后执行）
				const clearDone = () => {
					persist(todos.filter((value) => !value.done));
					setClearOpen(false);
				};
			...
			function HudPanel() {
				...
				return react.createElement("section", {
					...
					react.createElement("div", { className: "hud-panel-body", ref: bodyRef, onScroll: onPanelScroll },
						...
						// 便笺/待办：key 唯一（不可与 SessionModule 共用 sessionId——同级重复 key 会使
						// React diff 错乱、每次重渲染累积重复模块）；key 变化即按会话重挂载，
						// 内容与会话一一对应，切换会话互不串扰
			...
			ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				...
				react.useEffect(() => {
					...
					if (isBlank) {
						...
						// 回到真实会话：恢复打开（清除被动标记）；
						// 经 openHudDeferred 尊重 better-sidebar 互斥：侧边栏开着则不立即打开，
						// 延迟到其关闭后再恢复，避免「HUD 开 + 侧边栏开」共存
						// （面板被侧边栏遮挡 + 会话正文右侧 300px 空白占位）。
				...
				return react.createElement(react.Fragment, null,
					open
						? react.createElement(HudErrorBoundary, null, react.createElement(HudPanel, null))
						: null,
