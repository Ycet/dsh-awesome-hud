		...
		function apply(ctx) {
			...
			function persistNoteEntry(scope, value) {
				...
				try {
					...
					globalThis.localStorage?.setItem(NOTES_STORAGE_KEY, JSON.stringify(map));
				} catch { /* 存储不可用仅内存 */ }
				return payload;
			}
			...
			function NoteModule(props) {
				...
				const patchNote = (patch) => {
					setNote((prev) => {
						...
						const payload = persistNoteEntry(scope, next);
				...
				const startResize = (event) => {
					...
					const onMove = (moveEvent) => {
						...
						setNote((prev) => {
							...
							const payload = persistNoteEntry(scope, { ...prev, height: next });
							return { text: payload.text, height: payload.height, updatedAt: payload.updatedAt };
						});
					};
					const onUp = () => {
