...
/** 解析一行 --name-status 输出（M\tpath / A\tpath / R100\told\tnew）→ {status, path, oldPath}。 */
export function parseNameStatusLine(line) {
	const parts = String(line ?? "").split("\t");
	if (parts.length < 2) return null;
	const status = parts[0];
	if (!/^[AMDRCTUXB!?][0-9]{0,3}$/.test(status)) return null;
	const letter = status[0];
	const isRename = (letter === "R" || letter === "C") && parts.length > 2;
	return {
		status: letter,
		path: parts[parts.length - 1],
		oldPath: isRename ? parts[1] : null,
	};
}

/** 解析 git log 输出 → [{hash, parents[], refs[], subject, date, files[]}]；非法行跳过。
 *  提交头行含 0x1f 分隔；随后的 --name-status 行（状态符+TAB+路径）归属当前提交。 */
export function parseGitLogOutput(text) {
	...
	for (const line of String(text ?? "").split("\n")) {
		...
		if (fields.length >= 5 && /^[0-9a-f]{40}$/.test(fields[0])) {
		...
		// 形似提交头但非法（如哈希不合法）→ 终止当前提交的文件归属
		if (fields.length >= 5) { current = null; continue; }
		if (current === null) continue;
		const file = parseNameStatusLine(trimmed);
		if (file !== null) current.files.push(file);
	}
	return commits;
}
...
/**
 * 分页切片：git log 每页多取 1 条探测是否还有下一页。
 * 返回 { commits, hasMore }：超过 pageSize 条时截断为前 pageSize 条并标记 hasMore。
 */
export function slicePage(commits, pageSize = 80) {
	const list = Array.isArray(commits) ? commits : [];
	const hasMore = list.length > pageSize;
	return { commits: hasMore ? list.slice(0, pageSize) : list, hasMore };
}
