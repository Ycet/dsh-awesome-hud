...
/** 解析 tool/result 的调用配对键：真实键在 message.content[0].toolCallId；兼容旧结构 data.callId。 */
function callIdOfResult(event) {
	const message = event.data?.message;
	if (isRecord(message) && Array.isArray(message.content)) {
		for (const block of message.content) {
			if (isRecord(block) && typeof block.toolCallId === "string" && block.toolCallId !== "") return block.toolCallId;
		}
	}
...
/** 判定工具结果是否失败：data.error 存在，或结果块 isError=true。 */
...
export function scanPlanEvents(events) {
	...
	for (const event of events) {
		...
		if (event.type === "tool/result" && isRecord(event.data)) {
			...
			if (call === undefined) {
				results.set(callId, { resultError: isResultError(event) });
				continue;
			}
			call.hasResult = true;
			call.resultError = isResultError(event);
			continue;
		}
	}
