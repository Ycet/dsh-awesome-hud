...
- **git module**: refreshes every 5 seconds while the panel is open (immediately after write operations); the "git graph" popup shows the last 80 commits across all refs; the HEAD commit is marked with an enlarged white-filled, blue-stroked dot; changed files are grouped and shown with status letters colored by type — modified `M` yellow, added `A` blue, deleted `D` red, untracked `?` gray (renamed `R` / copied `C` blue):
  ...
  - **AI generate**: uses the model selected in the current session with the full staged diff (truncated over 16 KB) and generates the message per the built-in local git commit convention — `[YYMMDD] project-name vX.Y.Z: summary` (date = current system time; version bumped strictly by the rules: breaking change → major, backward-compatible feature → minor, fix/docs → patch; project name and current version come from the nearest package.json above the workspace)
