...
| git | Current branch, changed-file counts; changed files grouped into staged / changes (group names with gray counts, empty groups hidden); clicking a file row stages / unstages / reverts it; the "commit" button commits the staged area (manual input or AI-generated message using the session's model and AGENTS.md conventions) and "revert" discards all unstaged changes (destructive actions all require a confirmation dialog listing affected files); the "git graph" popup (last 80 commits across all refs); shown only within a git repository |
...
- **git module**: refreshes every 5 seconds while the panel is open (immediately after write operations); the "git graph" popup shows the last 80 commits across all refs; the HEAD commit is marked with an enlarged white-filled, blue-stroked dot; changed files are grouped and shown with status letters colored by type — modified `M` yellow, added `A` blue, deleted `D` red, untracked `?` gray (renamed `R` / copied `C` blue):
  - **Groups**: `staged` and `changes` (includes untracked) with a gray per-file count next to each group name; a group hides when it has no files
  ...
  - **commit**: disabled while nothing is staged; opens a dialog with the number of staged files to commit, a single-line input, AI generate and commit buttons; commit is disabled while the input is empty; only the staged area is committed
  - **AI generate**: uses the model selected in the current session, combining the full staged diff (truncated over 16 KB) and AGENTS.md files found by walking up from the workspace (plus the user-level `~/.dsh/AGENTS.md`, truncated over 32 KB) to produce the commit message — projects/users follow their own AGENTS conventions
  - **revert**: disabled when there is no tracked-file unstaged change; the confirmation dialog lists every file to be discarded; only unstaged changes of tracked files are reverted (untracked new files and the staged area are kept)
